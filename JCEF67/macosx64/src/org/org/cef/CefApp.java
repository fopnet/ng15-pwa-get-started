...
public class CefApp extends CefAppHandlerAdapter {
    public  class CefVersion {
        ...
        public CefVersion(int jcefCommitNo, int cefMajor, int cefMinor, int cefPatch,
    ...
    public HashSet<CefClient> clients_ = new HashSet<CefClient>();
     ...
     */
    public CefApp(String[] args, CefSettings settings) throws UnsatisfiedLinkError {
        super(args);
    ...
    public  void setSettings(CefSettings settings) throws IllegalStateException {
        if (getState() != CefAppState.NONE && getState() != CefAppState.NEW)
            throw new IllegalStateException("Settings can only be passed to CEF"
                    + " before createClient is called the first time.");
    ...
    public static  void setState(final CefAppState state) {
        synchronized (state_) {
            state_ = state;
        }
        // Execute on the AWT event dispatching thread.
     ...
     */
    public  synchronized void clientWasDisposed(CefClient client) {
     ...
     */
    public  void doMessageLoopWork(final long delay_ms) {
        // Execute on the AWT event dispatching thread.
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                ...
                // calls to DoMessageLoopWork().
                 long kMaxTimerDelay = 1000 / 30; // 30fps
     ...
     */
    public static  String getJcefLibPath() {
     ...
     */
    public static  String getCefFrameworkPath(String[] args) {
    ...
    public  static native boolean N_Startup(String pathToCefFramework);
    public  native boolean N_PreInitialize();
    public  native boolean N_Initialize(CefAppHandler appHandler, CefSettings settings);
    public  native void N_Shutdown();
    public  native void N_DoMessageLoopWork();
    public  native CefVersion N_GetVersion();
    public  native boolean N_RegisterSchemeHandlerFactory(
            String schemeName, String domainName, CefSchemeHandlerFactory factory);
